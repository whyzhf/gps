...
/*	@Cacheable(value = "getPolice",key="#p0"  ,unless="#result == null")
	...
	@Select("SELECT color,equip,equipCard,errorStatus,latitude,longitude,police,prisoner,stauts,taskId,taskName,uptime from outbound_gpslog  where taskId=#{taskId} ORDER by uptime")
    List<WSgpsData> getHisBySql(@Param("taskId") Integer taskId);
}
